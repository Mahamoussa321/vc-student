...
if (USE_KG_BOUNDARIES) {
  ...
  if (!file.exists(KG_ASCII)) stop("Missing KÃ¶ppen-Geiger ASCII file: ", KG_ASCII)
...
p_student_air <- ggplot() +
  ...
  scale_color_viridis_c(option = "viridis", limits = air_lim, breaks = air_breaks, oob = scales::squish, name = "Air threshold (Â°C)") +
...
p_scatter_air_facet <- ggplot(cmp_clim_main, aes(x = teacher_thr_Air_C, y = student_thr_Air_C)) +
  ...
  labs(x = "Teacher air threshold (Â°C)", y = "Student air threshold (Â°C)") +
