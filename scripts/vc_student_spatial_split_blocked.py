...
# ---------- Split settings ----------
SPLIT_MODE       = "spatial"   # {"spatial", "temporal", "random"}
TEST_SIZE        = 0.15
VAL_SIZE         = 0.15          # fraction of the full dataset
TEST_YEARS       = 2             # used only when SPLIT_MODE == "temporal"
DATE_CANDIDATES  = ["Date", "Timestamp", "date", "timestamp", "datetime"]
STATION_COL_CANDIDATES = [
    "Station_ID", "station_id", "StationID", "station", "Station",
    "station_name", "Station_Name", "SITE", "Site", "site",
    "USAF", "WBAN", "stid", "STID"
]
SPATIAL_GROUP_BY = "station_or_grid"  # {"station_or_grid", "station", "grid", "region"}
SPATIAL_GROUP_DECIMALS = 3             # legacy fallback when grid grouping is not used
SPATIAL_GRID_DEGREES = 2.0             # coarser fallback than rounded lat/lon for unseen-area testing
SPATIAL_BUFFER_KM = 75.0               # remove train/val groups within this distance of held-out test groups
SPATIAL_TARGET_TOL = 0.03              # acceptable deviation from requested row fraction when choosing groups
SPATIAL_MAX_SPLIT_ATTEMPTS = 250
SAVE_SPLIT_ASSIGNMENTS = True
SPATIAL_SPLIT_STYLE = "blocked_latlon"  # {"blocked_latlon", "random_groups"}
SPATIAL_TEST_SEED_MODE = "auto"         # auto tries geographic corners and keeps the best contiguous holdout
SPATIAL_VAL_SEED_MODE = "opposite"      # opposite uses the geographic opposite of the chosen test corner
SPATIAL_CORNER_CANDIDATES = ("north_west", "north_east", "south_west", "south_east")
...
BASE_RUN_TAG = "distill" if USE_DISTILLATION else "nodistill"
SPATIAL_SUFFIX = f"_{SPATIAL_SPLIT_STYLE}" if SPLIT_MODE == "spatial" else ""
RUN_TAG = f"{BASE_RUN_TAG}_{SPLIT_MODE}{SPATIAL_SUFFIX}"
...
#3 --------- Split data -------------------------------------------------------------------------------
def _positive_rate(df, y_col):
    return float(pd.to_numeric(df[y_col], errors="coerce").mean()) if len(df) else float("nan")
...
def _make_spatial_groups(df):
    work = df.copy()

    if SPATIAL_GROUP_BY == "region":
        work["_split_group"] = work[region_col].astype(str)
        desc = f"region column '{region_col}'"
        return work, desc

    station_col = next((c for c in STATION_COL_CANDIDATES if c in work.columns), None)
    if station_col is not None and SPATIAL_GROUP_BY in {"station_or_grid", "station"}:
        work["_split_group"] = work[station_col].astype(str)
        desc = f"station column '{station_col}'"
        return work, desc

    needed = ["Longitude", "Latitude"]
    if not set(needed).issubset(work.columns):
        raise ValueError(
            "SPLIT_MODE='spatial' requires either a station identifier column or both Longitude and Latitude."
        )

    lon = pd.to_numeric(work["Longitude"], errors="coerce")
    lat = pd.to_numeric(work["Latitude"], errors="coerce")

    if SPATIAL_GROUP_BY == "grid" or (SPATIAL_GROUP_BY == "station_or_grid" and station_col is None):
        grid_deg = float(SPATIAL_GRID_DEGREES)
        lon_bin = np.floor((lon + 180.0) / grid_deg)
        lat_bin = np.floor((lat + 90.0) / grid_deg)
        work["_split_group"] = (
            "grid_"
            + pd.Series(lat_bin, index=work.index).astype("Int64").astype(str)
            + "__"
            + pd.Series(lon_bin, index=work.index).astype("Int64").astype(str)
        )
        desc = f"{grid_deg:.2f}° latitude/longitude grid cells"
        return work, desc

    work["_split_group"] = lat.round(SPATIAL_GROUP_DECIMALS).astype(str) + "__" + lon.round(SPATIAL_GROUP_DECIMALS).astype(str)
    desc = f"rounded Latitude/Longitude at {SPATIAL_GROUP_DECIMALS} decimals"
    return work, desc


def _build_group_table(df):
    agg = {
        "n_rows": (y_var, "size"),
        "positive_rate": (y_var, lambda s: float(pd.to_numeric(s, errors="coerce").mean())),
    }
    if "Latitude" in df.columns:
        agg["group_lat"] = ("Latitude", lambda s: float(pd.to_numeric(s, errors="coerce").median()))
    if "Longitude" in df.columns:
        agg["group_lon"] = ("Longitude", lambda s: float(pd.to_numeric(s, errors="coerce").median()))
    group_df = df.groupby("_split_group", dropna=False).agg(**agg).reset_index()
    group_df["n_rows"] = group_df["n_rows"].astype(int)
    return group_df


def _choose_group_subset(group_df, target_frac, rng, max_attempts=250, tol=0.03):
    if not (0 < target_frac < 1):
        raise ValueError(f"target_frac must be in (0,1); got {target_frac}")
    if len(group_df) < 2:
        raise ValueError("Need at least two spatial groups to create a split.")

    weights = group_df["n_rows"].to_numpy(dtype=float)
    groups = group_df["_split_group"].to_numpy(dtype=str)
    total_rows = weights.sum()
    target_rows = target_frac * total_rows
    best = None

    for _ in range(max_attempts):
        perm = rng.permutation(len(group_df))
        cum = np.cumsum(weights[perm])
        k = int(np.searchsorted(cum, target_rows, side="left")) + 1
        k_candidates = sorted({max(1, k - 1), min(len(perm), k), min(len(perm), k + 1)})
        for kk in k_candidates:
            picked = perm[:kk]
            frac = float(weights[picked].sum() / total_rows)
            diff = abs(frac - target_frac)
            record = (diff, frac, set(groups[picked]))
...
def _opposite_seed_mode(seed_mode):
    mapping = {
    ...
    return mapping.get(seed_mode, "south_west")


def _seed_scores(group_df, seed_mode):
    lat = group_df["group_lat"].to_numpy(dtype=float)
    lon = group_df["group_lon"].to_numpy(dtype=float)
    lat_std = (lat - np.nanmean(lat)) / max(np.nanstd(lat), 1e-8)
    lon_std = (lon - np.nanmean(lon)) / max(np.nanstd(lon), 1e-8)

    if seed_mode == "north_west":
        return lat_std - lon_std
    if seed_mode == "north_east":
        return lat_std + lon_std
    if seed_mode == "south_west":
        return -lat_std - lon_std
    if seed_mode == "south_east":
        return -lat_std + lon_std
    if seed_mode == "north":
        return lat_std
    if seed_mode == "south":
...
def _choose_blocked_group_subset(group_df, target_frac, seed_mode="north_east"):
    if not (0 < target_frac < 1):
        raise ValueError(f"target_frac must be in (0,1); got {target_frac}")
    needed_cols = {"group_lon", "group_lat"}
    if not needed_cols.issubset(group_df.columns):
        raise ValueError("Blocked lat/lon spatial split requires group_lon and group_lat columns.")
    work = group_df.dropna(subset=["group_lon", "group_lat"]).reset_index(drop=True)
    if len(work) < 2:
        raise ValueError("Need at least two groups with coordinates to create a blocked spatial split.")

    weights = work["n_rows"].to_numpy(dtype=float)
    groups = work["_split_group"].to_numpy(dtype=str)
    total_rows = weights.sum()
    target_rows = target_frac * total_rows

    scores = _seed_scores(work, seed_mode)
    seed_idx = int(np.nanargmax(scores))
    seed_lon = float(work.loc[seed_idx, "group_lon"])
    seed_lat = float(work.loc[seed_idx, "group_lat"])

    d_km = _haversine_km(np.array([seed_lon]), np.array([seed_lat]),
                         work["group_lon"].to_numpy(dtype=float),
                         work["group_lat"].to_numpy(dtype=float)).reshape(-1)
    tie = np.arange(len(work), dtype=float) * 1e-9
    order = np.argsort(d_km + tie)
    cum = np.cumsum(weights[order])
    k = int(np.searchsorted(cum, target_rows, side="left")) + 1
    k_candidates = sorted({max(1, k - 1), min(len(order), k), min(len(order), k + 1)})
    ...
    for kk in k_candidates:
        ...
        frac = float(weights[picked].sum() / total_rows)
        diff = abs(frac - target_frac)
        compactness = float(np.average(d_km[picked], weights=weights[picked])) if len(picked) else float("inf")
        record = (diff, compactness, frac, set(groups[picked]), seed_mode)
    ...
    return {
        ...
        "seed_lon": seed_lon,
        "seed_lat": seed_lat,
        "compactness_km": best[1],
    }


def _choose_blocked_group_subset_auto(group_df, target_frac, seed_mode="auto"):
    if seed_mode != "auto":
        return _choose_blocked_group_subset(group_df, target_frac, seed_mode=seed_mode)

    best = None
    global_rate = float(np.average(group_df["positive_rate"].to_numpy(dtype=float),
                                   weights=group_df["n_rows"].to_numpy(dtype=float)))
    for mode in SPATIAL_CORNER_CANDIDATES:
        rec = _choose_blocked_group_subset(group_df, target_frac, seed_mode=mode)
        picked = group_df[group_df["_split_group"].isin(rec["groups"])]
        picked_rate = float(np.average(picked["positive_rate"].to_numpy(dtype=float),
                                       weights=picked["n_rows"].to_numpy(dtype=float)))
        score = (
            abs(rec["frac"] - target_frac),
            abs(picked_rate - global_rate),
            rec["compactness_km"],
        )
...
def _buffered_groups(group_df, held_out_groups, buffer_km):
    if buffer_km <= 0:
        return set(), 0
    needed_cols = {"group_lon", "group_lat"}
    if not needed_cols.issubset(group_df.columns):
        print("[warn] SPATIAL_BUFFER_KM > 0 but group coordinates are unavailable; skipping buffer.")
        return set(), 0

    meta = group_df[["_split_group", "group_lon", "group_lat"]].copy()
    meta = meta.dropna(subset=["group_lon", "group_lat"]).reset_index(drop=True)
    held = meta[meta["_split_group"].isin(held_out_groups)].reset_index(drop=True)
    cand = meta[~meta["_split_group"].isin(held_out_groups)].reset_index(drop=True)
    if held.empty or cand.empty:
        return set(), 0

    blocked = set()
    chunk = 512
    held_lon = held["group_lon"].to_numpy(dtype=float)
    held_lat = held["group_lat"].to_numpy(dtype=float)
    cand_lon = cand["group_lon"].to_numpy(dtype=float)
    cand_lat = cand["group_lat"].to_numpy(dtype=float)
    cand_ids = cand["_split_group"].to_numpy(dtype=str)

    for start in range(0, len(cand), chunk):
        stop = min(start + chunk, len(cand))
        dist = _haversine_km(cand_lon[start:stop], cand_lat[start:stop], held_lon, held_lat)
        is_blocked = np.any(dist <= buffer_km, axis=1)
        blocked.update(cand_ids[start:stop][is_blocked].tolist())
...
if SPLIT_MODE == "temporal":
    ...
    if date_col is None:
        raise ValueError(
            f"SPLIT_MODE='temporal' but none of DATE_CANDIDATES were found: {DATE_CANDIDATES}"
        )
    ...
    val_rel_size = VAL_SIZE / max(1e-12, (1.0 - len(test_df) / len(df)))
    val_rel_size = min(max(val_rel_size, 1e-6), 0.99)
    n_val = max(1, int(len(trainval_df) * val_rel_size))
    ...
    df, group_desc = _make_spatial_groups(raw.copy().reset_index(drop=True))
    df = df.dropna(subset=["_split_group", y_var, region_col]).reset_index(drop=True)
    df["_split_group"] = df["_split_group"].astype(str)

    group_df = _build_group_table(df)

    if SPATIAL_SPLIT_STYLE == "blocked_latlon":
        test_pick = _choose_blocked_group_subset_auto(
            group_df,
            target_frac=TEST_SIZE,
            seed_mode=SPATIAL_TEST_SEED_MODE,
        )
        test_groups = set(test_pick["groups"])
        approx_test_frac = float(test_pick["frac"])
        test_seed_mode = test_pick["seed_mode"]
    else:
        rng = np.random.default_rng(SEED)
        test_groups, approx_test_frac = _choose_group_subset(
            group_df,
            target_frac=TEST_SIZE,
            rng=rng,
            max_attempts=SPATIAL_MAX_SPLIT_ATTEMPTS,
            tol=SPATIAL_TARGET_TOL,
        )
        test_seed_mode = "random_groups"

    buffer_groups, n_buffer_groups = _buffered_groups(group_df, test_groups, SPATIAL_BUFFER_KM)

    test_mask = df["_split_group"].isin(test_groups)
    buffer_mask = df["_split_group"].isin(buffer_groups)
    keep_mask = ~(test_mask | buffer_mask)

    test_df = df.loc[test_mask].copy()
    retained_df = df.loc[keep_mask].copy()
    excluded_buffer_df = df.loc[buffer_mask].copy()

    retained_group_df = group_df[~group_df["_split_group"].isin(test_groups | buffer_groups)].reset_index(drop=True)
    if retained_group_df.empty:
        raise ValueError("Spatial split failed: no groups remain after applying the test holdout and buffer.")

    retained_frac = len(retained_df) / max(1, len(df))
    val_rel_size = VAL_SIZE / max(1e-12, retained_frac)
    val_rel_size = min(max(val_rel_size, 1e-6), 0.99)

    if SPATIAL_SPLIT_STYLE == "blocked_latlon":
        if SPATIAL_VAL_SEED_MODE == "opposite":
            val_seed_mode = _opposite_seed_mode(test_seed_mode)
        else:
            val_seed_mode = SPATIAL_VAL_SEED_MODE
        val_pick = _choose_blocked_group_subset_auto(
            retained_group_df,
            target_frac=val_rel_size,
            seed_mode=val_seed_mode,
        )
        val_groups = set(val_pick["groups"])
        approx_val_frac_within_retained = float(val_pick["frac"])
    else:
        val_groups, approx_val_frac_within_retained = _choose_group_subset(
            retained_group_df,
            target_frac=val_rel_size,
            rng=np.random.default_rng(SEED + 1),
            max_attempts=SPATIAL_MAX_SPLIT_ATTEMPTS,
            tol=max(SPATIAL_TARGET_TOL, 0.05),
        )
        val_seed_mode = "random_groups"

    val_mask = retained_df["_split_group"].isin(val_groups)
    val_df = retained_df.loc[val_mask].copy()
    train_df = retained_df.loc[~val_mask].copy()

    train_groups = set(train_df["_split_group"].unique())
    val_groups   = set(val_df["_split_group"].unique())
    test_groups  = set(test_df["_split_group"].unique())
    buffer_groups = set(excluded_buffer_df["_split_group"].unique())

    assert train_groups.isdisjoint(val_groups)
    assert train_groups.isdisjoint(test_groups)
    assert val_groups.isdisjoint(test_groups)

    print(f"[spatial] Grouping by {group_desc}")
    print(f"[spatial] Split style: {SPATIAL_SPLIT_STYLE}")
    if SPATIAL_SPLIT_STYLE == "blocked_latlon":
        print(f"[spatial] Test seed mode: {test_seed_mode}")
        print(f"[spatial] Val seed mode: {val_seed_mode}")
    print(f"[spatial] Buffer: {SPATIAL_BUFFER_KM:.1f} km")
    print("[spatial] Train/Val/Test shapes:", train_df.shape, val_df.shape, test_df.shape)
    print("[spatial] Excluded by spatial buffer:", excluded_buffer_df.shape)
    print("[spatial] Unique groups Train/Val/Test/Buffered:",
          len(train_groups), len(val_groups), len(test_groups), len(buffer_groups))
    print("[spatial] Positive rates Train/Val/Test:",
          f"{_positive_rate(train_df, y_var):.4f}",
          f"{_positive_rate(val_df, y_var):.4f}",
          f"{_positive_rate(test_df, y_var):.4f}")
    print("[spatial] Approx row fractions Test/Val-within-retained:",
          f"{approx_test_frac:.4f}", f"{approx_val_frac_within_retained:.4f}")
    print("[spatial] Buffered groups removed:", n_buffer_groups)

    if SAVE_SPLIT_ASSIGNMENTS:
        split_assignments = pd.concat([
            train_df.assign(split_set="train", split_role="model"),
            val_df.assign(split_set="val", split_role="model"),
            test_df.assign(split_set="test", split_role="held_out"),
            excluded_buffer_df.assign(split_set="buffer_excluded", split_role="excluded"),
        ], ignore_index=True)
        split_cols = [c for c in ["split_set", "split_role", "_split_group", region_col, y_var, "Longitude", "Latitude", "Elevation"] if c in split_assignments.columns]
        split_path = RUN_DIR / f"split_assignments__{RUN_TAG}.csv"
        split_assignments[split_cols].to_csv(split_path, index=False)
        print(f"[ok] Saved split assignments → {split_path}")

    for _df in (train_df, val_df, test_df):
        _df.drop(columns=["_split_group"], inplace=True, errors="ignore")

else:
    train_df, tmp_df = train_test_split(raw, test_size=TEST_SIZE + VAL_SIZE,
                                        random_state=SEED, stratify=raw[y_var])
    val_rel_size = VAL_SIZE / (TEST_SIZE + VAL_SIZE)
    val_df, test_df = train_test_split(tmp_df, test_size=1.0 - val_rel_size,
                                       random_state=SEED, stratify=tmp_df[y_var])
